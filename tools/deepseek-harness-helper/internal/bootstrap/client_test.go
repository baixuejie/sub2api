...
func TestClientExchangeAndReport(t *testing.T) {
	...
	defer server.Close()
	mux.HandleFunc("/api/v1/deepseek-harness/exchange", func(w http.ResponseWriter, r *http.Request) {
		...
		_ = json.NewEncoder(w).Encode(Envelope[Task]{Data: Task{
			OperationID: "op", EventToken: "event", StatusURL: server.URL + "/api/v1/deepseek-harness/sessions/op/events",
	...
	mux.HandleFunc("/api/v1/deepseek-harness/sessions/op/events", func(w http.ResponseWriter, r *http.Request) {
	...
	launch := LaunchRequest{Server: server.URL, Ticket: "ticket", OperationID: "op"}
	client := NewClient()
	task, err := client.Exchange(context.Background(), launch)
	if err != nil {
		t.Fatal(err)
	}
	if task.ServerOrigin != server.URL || task.APIKey != "secret" {
