...
func TestInstallUsesFixedArguments(t *testing.T) {
	...
	run := &installRunner{}
	_, err := Install(context.Background(), run, Environment{NodePath: "node", NPMPath: "npm"}, paths, "0.1.0-rc.6")
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, version := range []string{"0.1.0-rc.7", "latest; calc.exe"} {
		if _, err := Install(context.Background(), run, Environment{NodePath: "node", NPMPath: "npm"}, paths, version); err == nil {
