...
2. 发布并签名 `tools/deepseek-harness-helper/` 的六平台产物。默认下载版本为 `dsh-helper-v0.1.5`。
3. 确认 Redis 可用，然后再启用 Feature Flag `ext_deepseek_harness_enabled`。

可通过环境变量覆盖 Helper 发布位置：

```text
DEEPSEEK_HARNESS_HELPER_RELEASE_BASE_URL=https://downloads.example.com/dsh-helper-v0.1.5
